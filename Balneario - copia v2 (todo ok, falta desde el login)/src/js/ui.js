...
function showPricingModal(type) {
  ...
  // Event listeners
  document.getElementById('close-pricing-modal').addEventListener('click', () => modal.remove());
    ...
    <div class="modal modal-payments">
      ...
      <div class="modal-body">
        ...
        <div class="payment-rental-info">
          ...
          <p><strong>Período:</strong> ${formatDateShort(rental.startDate)} → ${formatDateShort(rental.endDate)}</p>
        </div>
              ...
              ? summary.payments.map(payment => `
                  <div class="payment-item" data-payment-id="${payment.id}">
                    <div class="payment-item-header">
                      <span class="payment-date">${formatDateShort(payment.paymentDate)}</span>
        ...
        ${(() => {
          ...
          return allPayments.map((p, index) => `
            <div class="receipt-row payment-history-item ${p.id === payment.id ? 'current-payment' : ''}">
              <span class="receipt-label">${index + 1}. ${formatDateShort(p.paymentDate)} - ${PAYMENT_METHODS[p.paymentMethod]?.icon || '💵'}</span>
...
function showNewPoolEntryModal() {
  ...
  modal.className = 'modal-overlay';
  modal.innerHTML = `
    <div class="modal modal-large pool-modal">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <form id="pool-entry-form">
          ...
          <div class="form-section">
            ...
            <input type="number" id="pool-people-count" min="1" max="20" value="1" required>
          ...
          <div class="pool-price-summary">
            ...
            <div class="price-breakdown">
              <div class="price-row">
                ...
                <span id="pool-base-price-display">${formatCurrency(config.prices.dayPass)}</span>
              </div>
              ...
              <div class="price-row total-row">
                ...
                <span id="pool-total-price">${formatCurrency(config.prices.dayPass)}</span>
              </div>
            </div>
          </div>
  ...
  // Event listeners
  document.getElementById('close-pool-entry-modal').addEventListener('click', () => {
    modal.remove();
  });
  
  document.getElementById('cancel-pool-entry').addEventListener('click', () => {
    modal.remove();
  });
  ...
  document.getElementById('pool-people-count').addEventListener('input', updatePoolPriceSummary);
...
function updatePoolPriceSummary() {
  ...
  const peopleCount = parseInt(document.getElementById('pool-people-count')?.value) || 1;
  ...
  const priceInfo = calculatePoolEntryPrice(peopleCount, entryType, numberOfDays);
  
  // Actualizar displays
  document.getElementById('pool-base-price-display').textContent = 
    `${formatCurrency(priceInfo.basePrice)} × ${peopleCount} × ${numberOfDays} = ${formatCurrency(priceInfo.subtotal)}`;
...
function confirmPoolEntry(foundClient = null) {
  ...
  const peopleCount = parseInt(document.getElementById('pool-people-count').value);
  ...
  if (peopleCount < 1) {
    showNotification('La cantidad de personas debe ser mayor a 0', 'error');
    return false;
  }
  ...
  if (entryType === 'day') {
    ...
    // Validar capacidad
    const validation = canAddPoolEntry(date, peopleCount);
    if (!validation.valid) {
    ...
    const validation = validateStayPassDates(dates, peopleCount);
    if (!validation.valid) {
      showNotification(validation.message, 'error');
      return false;
    }
